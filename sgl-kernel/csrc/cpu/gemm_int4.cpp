  ...
  for (int m = 0; m < M; ++m) {
    ...
    for (int n = 0; n < N; ++n) {
      ...
      if constexpr (sym_quant_act) {
        ...
        dq_val = (float)(input[m * ldi + n] - a_zp * (comp_b ? comp_b[n] : 0)) * a_scale * scale_b[n];
      }
      if constexpr (accum) {
        output[m * ldo + n] += dq_val;
      } else {
...
void _dequant_gemm_accum_small_M(
  ...
  for (int m = 0; m < M; ++m) {
    ...
    for (int n = 0; n < N; n += vl_s32) {
      for(int i = 0; i < (int)vl_s32 && (n + i) < N; ++i) {
         int col = n + i;
         int32_t sum = 0;
         int32_t comp = 0;
         ...
         for (int k = 0; k < K; ++k) {
             ...
             if constexpr (sym_quant_act) sum += ((int8_t)a_val) * b_val;
             else { sum += a_val * b_val; comp += b_val; }
         }
         float c_val;
         if constexpr (sym_quant_act) c_val = (float)sum;
         else c_val = (float)(sum - a_zp * comp);
...
void _dequant_gemm_accum_small_M(
  ...
  for (int m = 0; m < M; ++m) {
    float a_scale = scales_a[m];
    int32_t a_zp = 0;
    if constexpr (!sym_quant_act) a_zp = qzeros_a[m];
...
void _dequant_gemm_accum(
  ...
  if (!use_brgemm) {
    switch (M) {
      case 1: CALL_DEQUANT_GEMM_ACCUM_SMALL_M(1); break;
      case 2: CALL_DEQUANT_GEMM_ACCUM_SMALL_M(2); break;
      case 3: CALL_DEQUANT_GEMM_ACCUM_SMALL_M(3); break;
      case 4: CALL_DEQUANT_GEMM_ACCUM_SMALL_M(4); break;
      default: TORCH_CHECK(false, "tinygemm_kernel: unexpected M for AVX path!");
    }
    return;
  }
  ...
  if (use_brgemm) {
    int32_t C_i32[M * N];
    at::native::cpublas::brgemm(M, N, K, lda, N, N, false, A_ptr, dqB, C_i32, true);
    _dequant_and_store<true, N, sym_quant_act>(C, C_i32, scales_a, qzeros_a, scales_b, compensation, M, N, ldc, 1);
  }
#else
  if (M <= 4) {
    switch (M) {
  ...
  _dequant_and_store<true, N, sym_quant_act>(C, C_i32.data(), scales_a, qzeros_a, scales_b, compensation, M, N, ldc, 1);
#endif
}
...
#endif
      for (; j < N; ++j) y_buf[i * N + j] = bias_ptr[j];
    }
  } else {
    for (int i = 0; i < m; ++i) memset(y_buf + i * N, 0, sizeof(float) * N);
  }
}
...
#endif
      for (; j < N; ++j) c_ptr[i * lda + j] = y_buf[i * N + j];
...
#endif
      for (; j < N; ++j) c_ptr[i * lda + j] = at::BFloat16(y_buf[i * N + j]);
...
#endif
      for (; j < N; ++j) c_ptr[i * lda + j] = at::Half(y_buf[i * N + j]);
    } else {
...
void fill_val_stub(int32_t* __restrict__ output, int32_t value, int64_t size) {
  ...
  int64_t d;
#pragma GCC unroll 4
  for (d = 0; d <= size - VecSize; d += VecSize) fill_val_vec.store(output + d);
  for (; d < size; ++d) output[d] = value;
}

template <typename act_dtype, typename out_dtype, bool sym_quant_act, int64_t block_n, int64_t ldb_val>
    ...
    for (const auto i : c10::irange(begin, end)) {
      ...
      for (int mci = mc; mci < mc_end; ++mci) {
        ...
        for (int kci = 0; kci < Kc; ++kci) {
          int32_t* compensation_ptr = sym_quant_act ? nullptr : (int32_t*)(void*)(weight + (nc * Kc + kci) * (block_n * (_block_k / 2 + sizeof(int32_t))) + _block_k * block_n / 2);
          _dequant_gemm_accum<block_n, ldb_val, sym_quant_act>(
              C_tmp, (uint8_t*)input + mci * block_m * K + kci * _block_k, input_scales + mci * block_m, input_qzeros + mci * block_m,
              weight + (nc * Kc + kci) * (block_n * (_block_k / 2 + sizeof(int32_t))),
              weight_scales + nc * block_n * num_groups + kci / block_per_group * block_n,
              weight_qzeros + nc * block_n * num_groups + kci / block_per_group * block_n,
              compensation_ptr, dqB_tmp, m_size, _block_k, K, block_n, use_brgemm);
        }
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> convert_int4_weight_packed_with_compensation(
  ...
  if (new_scales.dim() == 1) new_scales.unsqueeze_(1);
  new_scales = new_scales.to(at::kFloat);
  if (new_qzeros.dim() == 1) new_qzeros.unsqueeze_(1);
  ...
  auto weight_sub_qzero = weight.view({Nc, block_n, G, -1}).to(at::kInt) - new_qzeros.view({Nc, block_n, G, -1}).to(at::kInt);
  ...
  at::parallel_for(0, num_blocks, 1, [&](int64_t begin, int64_t end) {
    for (const auto i : c10::irange(begin, end)) {
      ...
      int32_t* comp_out_prt = (int32_t*)(void*)(blocked_weight_ptr + i * block_n * (_block_k / 2 + sizeof(int32_t)) + _block_k * block_n / 2);
      constexpr int n_group_size = 8;
      constexpr int vnni_size = 4;
      int n_group = block_n / n_group_size;
      ...
      for (int nb = 0; nb < block_n; nb++) comp_out_prt[nb] = comp_in_prt[nb];
    }
  });
  return std::make_tuple(std::move(blocked_weight), std::move(blocked_scales), std::move(blocked_qzeros));
}

std::tuple<at::Tensor, at::Tensor> autoawq_to_int4pack(at::Tensor qweight, at::Tensor qzeros) {
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> convert_weight_packed_scale_zp(at::Tensor qweight, at::Tensor qzeros, at::Tensor scales) {
  ...
  if (_qweight.dim() == 3) {
    ...
    _qweight = blocked_weight; _scales = blocked_scales; _qzeros = blocked_qzeros;
  } else {
    auto res_ = convert_int4_weight_packed_with_compensation(_qweight, _scales, _qzeros);
    _qweight = std::get<0>(res_); _scales = std::get<1>(res_); _qzeros = std::get<2>(res_);
  }
...
at::Tensor int4_scaled_mm_cpu_with_quant(const at::Tensor& input, const at::Tensor& weight, const at::Tensor& weight_scales, const at::Tensor& weight_qzeros, const std::optional<at::Tensor>& bias, at::ScalarType output_dtype) {
  ...
  TORCH_CHECK(N == Nc * block_n_val, "DA8W4: weight and input shapes mismatch");
...
template <typename scalar_t>
void tinygemm_kernel(scalar_t* C, float* C_temp, const uint8_t* A, const float* scales_a, const int32_t* qzeros_a, const uint8_t* B, const float* scales_b, const int8_t* qzeros_b, const int32_t* compensation, int8_t* dqB_tmp, int64_t M, int64_t K, int64_t lda, int64_t ldc_f, int64_t ldc_s, bool store_out, bool use_brgemm) {
  if (ldc_f == 128) _dequant_gemm_accum<128, 64, false>(C_temp, A, scales_a, qzeros_a, B, scales_b, qzeros_b, compensation, dqB_tmp, M, K, lda, ldc_f, use_brgemm);
  else if (ldc_f == 256) _dequant_gemm_accum<256, 128, false>(C_temp, A, scales_a, qzeros_a, B, scales_b, qzeros_b, compensation, dqB_tmp, M, K, lda, ldc_f, use_brgemm);
  else if (ldc_f == 16) _dequant_gemm_accum<16, 8, false>(C_temp, A, scales_a, qzeros_a, B, scales_b, qzeros_b, compensation, dqB_tmp, M, K, lda, ldc_f, use_brgemm);
  else if (ldc_f == 32) _dequant_gemm_accum<32, 16, false>(C_temp, A, scales_a, qzeros_a, B, scales_b, qzeros_b, compensation, dqB_tmp, M, K, lda, ldc_f, use_brgemm);
  else if (ldc_f == 64) _dequant_gemm_accum<64, 32, false>(C_temp, A, scales_a, qzeros_a, B, scales_b, qzeros_b, compensation, dqB_tmp, M, K, lda, ldc_f, use_brgemm);
  else TORCH_CHECK(false, "tinygemm_kernel (int4): unsupported N=", ldc_f);
  if (store_out) { for (int64_t m = 0; m < M; ++m) copy_stub<scalar_t>(C + m * ldc_s, C_temp + m * ldc_f, ldc_f); }
}

#define INSTANTIATE_TINYGEMM_TEMPLATE(TYPE) \
  template void tinygemm_kernel<TYPE>(TYPE * C, float* C_temp, const uint8_t* A, const float* scales_a, const int32_t* qzeros_a, const uint8_t* B, const float* scales_b, const int8_t* qzeros_b, const int32_t* compensation, int8_t* dqB_tmp, int64_t M, int64_t K, int64_t lda, int64_t ldc_f, int64_t ldc_s, bool store_out, bool use_brgemm)

INSTANTIATE_TINYGEMM_TEMPLATE(at::BFloat16);
INSTANTIATE_TINYGEMM_TEMPLATE(at::Half);

at::Tensor int4_scaled_mm_cpu(at::Tensor& x, at::Tensor& w, at::Tensor& w_zeros, at::Tensor& w_scales, std::optional<at::Tensor> bias) {
