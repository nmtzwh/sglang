...
template <typename scalar_t>
void rmsnorm_residual_scalar_kernel_impl(
    scalar_t* __restrict__ output,
    const scalar_t* __restrict__ input,
    const scalar_t* __restrict__ weight,
  ...
  constexpr int kVecSize = bVec::size();
  const fVec scalar_fvec = fVec(scalar);
  at::parallel_for(0, batch_size, 0, [&](int64_t begin, int64_t end) {
    for (int64_t i = begin; i < end; ++i) {
      scalar_t* __restrict__ out_ptr = output + i * hidden_size;
      const scalar_t* __restrict__ input_ptr = input + i * input_strideN;
      const scalar_t* __restrict__ residual_ptr = residual + i * residual_strideN;
...
#pragma GCC unroll 4
      for (d = 0; d <= hidden_size - kVecSize; d += kVecSize) {
        ...
        x_fvec0 = (x_fvec0 * scale_fvec * w_fvec0 + r_fvec0) * scalar_fvec;
        x_fvec1 = (x_fvec1 * scale_fvec * w_fvec1 + r_fvec1) * scalar_fvec;
...
#pragma GCC unroll 4
      for (; d < hidden_size; ++d) {
        ...
        out_ptr[d] = static_cast<scalar_t>(
            (x_val * rsqrt_var * w_val + r_val) * scalar);
      }
    }
  });
}
...
at::Tensor gemma4_rmsnorm_residual_cpu(
    at::Tensor& input,
    at::Tensor& weight,
    at::Tensor& residual,
    double scalar,
    double eps) {
  RECORD_FUNCTION(
      "sgl-kernel::gemma4_rmsnorm_residual_cpu",
      std::vector<c10::IValue>({input, weight, residual}));
  ...
  at::Tensor output = at::empty_like(input);
  int64_t input_strideN = input.stride(0);
  int64_t residual_strideN = residual.stride(0);
  float scalar_f = static_cast<float>(scalar);

  AT_DISPATCH_REDUCED_FLOATING_TYPES(
      input.scalar_type(), "gemma4_rmsnorm_residual_kernel", [&] {
        rmsnorm_residual_scalar_kernel_impl<scalar_t>(
            output.data_ptr<scalar_t>(),
            input.data_ptr<scalar_t>(),
            weight.data_ptr<scalar_t>(),
